...
class GoogleSpreadsheetWorksheet {
  ...
  async loadHeaderRow(rowNumber = 1) {
    const rows = await this.getCellsInRange(`A${rowNumber}:${this.lastColumnLetter}${rowNumber}`);
    if (!rows) {
      throw new Error('No values in the header row - fill the first row with header values before trying to interact with rows or pass a different header row number');
    }
    this.headerValues = _.map(rows[0], (header) => header.trim());

    // go through the headers, if one of them repeats, add a number to the end
    const headerCountMap = new Map();

    for (let i = 0; i < this.headerValues.length; i++) {
      const header = this.headerValues[i];

      let headerOccurence = headerCountMap.get(header);
      headerCountMap.set(header, (headerOccurence !== undefined) ? headerOccurence + 1 : 0);
      if (headerCountMap.get(header) > 0) {
        this.headerValues[i] = `${header} (${headerCountMap.get(header)})`;
      }
    }

    if (!_.compact(this.headerValues).length) {
      throw new Error('All your header cells are blank - fill the first row with header values before trying to interact with rows or pass a different header row number');
    }
